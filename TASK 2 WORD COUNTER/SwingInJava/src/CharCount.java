...
JLabel lb1,lb2;
JTextArea ta;
JButton btn,pad,text;

CharCount()
{
 super("Char Word Count Tool - Nannu");
 ...
 add(lb1);add(lb2);add(ta);add(btn);add(pad);add(text);  
   
 setSize(400,400);  
 setLayout(null);//using no layout manager  
 setVisible(true);  
 setDefaultCloseOperation(EXIT_ON_CLOSE);  //to terminate the profram from cmd when user close the windows
}  
